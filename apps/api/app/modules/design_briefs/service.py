...
def start_intake(
    ...
    given a won client, creates a new project (INTAKE stage) plus its
    brief in one step, pre-filled with whatever intake info was
    collected up front. A client can have more than one project over
    time, so this always creates a fresh project rather than reusing one.
    """
