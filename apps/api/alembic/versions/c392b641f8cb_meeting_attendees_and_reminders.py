...
def upgrade() -> None:
    ...
    # Already created explicitly above. create_type=False only suppresses the
    # implicit CREATE TYPE on generic sa.Enum when it's the PG dialect's own
    # ENUM class — plain sa.Enum ignores the flag and creates it again anyway,
    # so use postgresql.ENUM explicitly for the column reference.
    reminder_channel_column_type = postgresql.ENUM(
        'IN_APP', name='meeting_reminder_channel', create_type=False
    )
    ...
    op.create_table(
        ...
        sa.Column('channel', reminder_channel_column_type, nullable=False, server_default='IN_APP'),
