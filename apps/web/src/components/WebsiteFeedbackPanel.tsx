...
export function WebsiteFeedbackPanel({ projectId, websiteId }: { projectId: string; websiteId?: string }) {
  ...
  return (
    <div className="mt-8 border-t border-neutral-200 pt-6">
      <h2 className="text-sm font-semibold text-neutral-900">Client feedback</h2>
      {error && <p className="mt-2 text-sm text-red-600">{error}</p>}

      <div className="mt-4 space-y-3">
        {items === null && <p className="text-sm text-neutral-500">Loading…</p>}
        {items !== null && items.length === 0 && (
          <p className="text-sm text-neutral-500">No feedback yet — share a preview link to start collecting it.</p>
        )}
        {items?.map((item) => (
          <div
            key={item.id}
            className={`rounded-md border p-3 text-sm ${TYPE_STYLES[item.feedback_type] ?? "border-neutral-200"}`}
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <span className="font-medium text-neutral-900">{TYPE_LABELS[item.feedback_type]}</span>
                {item.page_slug !== null && (
                  <span className="text-xs text-neutral-500">on {item.page_slug === "" ? "Home" : item.page_slug}</span>
                )}
                <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${STATUS_STYLES[item.status]}`}>{item.status}</span>
              </div>
              <span className="text-xs text-neutral-400">{new Date(item.created_at).toLocaleString()}</span>
            </div>

            <p className="mt-2 text-neutral-800">{item.message}</p>
            ...
            <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
              ...
              {item.resolved_by_user_name && (
                <p className="text-xs text-neutral-500">by {item.resolved_by_user_name}</p>
              )}
            </div>
          </div>
        ))}
