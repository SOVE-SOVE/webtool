...
import { monthKey, parseMonthKey, useUrlState } from "@/lib/todayState";
import { notifyAfter, useOnTodayDataChanged } from "@/lib/todaySync";
import {
  api,
  ApiError,
  MEETING_TYPES,
...
/**
 * The Calendar workspace's actual content — everything the standalone
 * `/dashboard/calendar` route rendered below its own `<PageHeader>`,
 * extracted so the Today workspace's Calendar tab can embed it without
 * a second, duplicate "Calendar" header stacked under "Today"'s. The
 * standalone route (kept working, unchanged in behaviour — see
 * `calendar/page.tsx`) still wraps this in its own `<PageHeader>`; this
 * component owns no header and no outer page padding of its own.
 *
 * The primary action ("Schedule meeting") moved from that removed
 * header's `actions` slot into this component's own top row — the same
 * place Clients' own tab components each put their primary action.
 */
export function CalendarView() {
  const today = useMemo(() => new Date(), []);
  // The visible month lives in the URL (?cm=YYYY-MM, omitted for the
  // current month) so it survives a visit to a linked record and Back.
  const currentMonth = monthKey(today);
  const [monthParam, setMonthParam] = useUrlState("cm", currentMonth);
  const cursor = useMemo(() => parseMonthKey(monthParam) ?? parseMonthKey(currentMonth)!, [monthParam, currentMonth]);
  const shiftMonth = (delta: number) => setMonthParam(monthKey(new Date(cursor.getFullYear(), cursor.getMonth() + delta, 1)));
  ...
  useOnTodayDataChanged("calendar", load);
  useEffect(() => {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      await notifyAfter(api.createMeeting({
        title,
  ...
  async function handleStatusChange(status: MeetingStatus) {
    ...
    setMeetingError(null);
    try {
      const updated = await notifyAfter(api.updateMeeting(selectedMeeting.id, { status }), "calendar");
